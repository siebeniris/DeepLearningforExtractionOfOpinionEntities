...
filepath='experiments/bilstm_embed/trainable/nocrf/02'
import sys
...
# model
model = Sequential()
model.add(Embedding(input_dim= n_symbols, output_dim= embed_size, trainable=True, input_length=maxlen, weights=[embedding_weights], mask_zero = True))
model.add(Dropout(rate = 0.2))
model.add(Bidirectional(LSTM(hidden_units, return_sequences=True,kernel_initializer='random_uniform'  )))
model.add(Dropout(rate = 0.2))
model.add(Bidirectional(LSTM(hidden_units, return_sequences=True,kernel_initializer='random_uniform'  )))
model.add(Dropout(rate = 0.2))
model.add(Bidirectional(LSTM(hidden_units, return_sequences=True, kernel_initializer='random_uniform'  )))
...
model.add(TimeDistributed(Dense(7, kernel_initializer='random_uniform',activation='softmax')))
# compile model
